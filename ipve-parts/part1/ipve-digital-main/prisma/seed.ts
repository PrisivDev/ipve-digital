...
// Load environment variables - override existing ones with .env.local values
config({ path: '.env.local', override: true });

// Seed uses limited connection pool to avoid Supabase pooler limits
...
async function seedRBAC() {
  ...
  const permData = ALL_PERMS.map(p => ({
    module: (MODULE_MAP[p.module] ?? 'LMS') as 'CRM' | 'ERP' | 'LMS' | 'SETTINGS' | 'REPORTS',
    action: p.action.toUpperCase() as 'CREATE' | 'READ' | 'UPDATE' | 'DELETE' | 'EXPORT' | 'VALIDATE',
    resource: p.resource,
  }));
  ...
  if (rolePermData.length > 0) {
    // createMany doesn't support skipDuplicates for unique constraints well,
    // so delete and recreate
    await db.rolePermission.deleteMany();
    await db.rolePermission.createMany({ data: rolePermData });
  }

  console.log(`✅ ${ALL_PERMS.length} permissions RBAC créées (${rolePermData.length} mappings)`);
}

// Shared transaction timeout — 5 minutes to accommodate large batches
...
async function seed() {
  ...
  // ========================
  // Batch 1: 6 RÔLES (all independent upserts, single connection)
  // ========================
  ...
  console.log('✅ 6 rôles créés');

  // ========================
  // Pre-compute: all 6 password hashes (CPU-bound — no DB needed)
  // Must be resolved BEFORE putting into transaction array
  // ========================
  ...
  // ========================
  // Batch 2: 6 UTILISATEURS (all independent upserts, single connection)
  // ========================
  ...
  // ========================
  // Batch 3: Année académique + 2 périodes (all independent — academicYearId is a known string)
  // ========================
  ...
  // Interactive transaction — levels need filiere IDs (auto-generated)
  // All 9 operations share one connection
  // ========================
  const { filieres, levels } = await db.$transaction(async (tx) => {
    ...
    const f2 = await tx.filiere.upsert({
      ...
      create: {
        code: 'GECO-LIC',
        name: 'Gestion & Commerce (Licence)',
        description: 'Comptabilité, Marketing, Droit des affaires',
        durationYears: 3,
        isActive: true,
      },
    });
    const f3 = await tx.filiere.upsert({
      where: { code: 'MARKET-LIC' },
      update: {},
      create: {
        code: 'MARKET-LIC',
        name: 'Marketing Digital (Licence)',
        description: 'Marketing digital, Communication, E-commerce',
        durationYears: 3,
        isActive: true,
      },
    });

    // Create 6 niveaux (depend on filiere IDs)
    ...
    const l3 = await tx.level.upsert({
      where: { id: 'level-geco-l1' },
      update: {},
      create: {
        id: 'level-geco-l1',
        name: 'L1 Gestion & Commerce',
        filiereId: f2.id,
        yearNumber: 1,
        tuitionFee: 360000,
      },
    ...
    const l4 = await tx.level.upsert({
      where: { id: 'level-geco-l2' },
      update: {},
      create: {
        id: 'level-geco-l2',
        name: 'L2 Gestion & Commerce',
        filiereId: f2.id,
        yearNumber: 2,
        tuitionFee: 360000,
      },
    ...
    const l5 = await tx.level.upsert({
      where: { id: 'level-market-l1' },
      update: {},
      create: {
        id: 'level-market-l1',
        name: 'L1 Marketing Digital',
        filiereId: f3.id,
        yearNumber: 1,
        tuitionFee: 390000,
      },
    ...
    const l6 = await tx.level.upsert({
      where: { id: 'level-market-l2' },
      update: {},
      create: {
        id: 'level-market-l2',
        name: 'L2 Marketing Digital',
        filiereId: f3.id,
        yearNumber: 2,
        tuitionFee: 390000,
      },
  ...
  console.log('✅ 3 filières et 6 niveaux créés');

  // ========================
  // Batch 5: 9 MATIÈRES (all independent upserts, single connection)
  // ========================
  await db.$transaction([
    db.subject.upsert({
      where: { code: 'INF101' },
  ...
  console.log('✅ 9 matières créées');

  // ========================
  // Batch 6: 20 étudiants + 4 classes + affectations
  // Interactive transaction — class assignments need both student and class IDs
  // All 44 operations (20 creates + 4 creates + 20 updates) share one connection
  // ========================
  const studentData = [
    { fn: 'Abdoulaye', ln: 'Koné', g: 'MALE' as const, f: 0, l: 0, hasUser: true },
    { fn: 'Fatoumata', ln: 'Traoré', g: 'FEMALE' as const, f: 0, l: 0, hasUser: false },
    { fn: 'Ibrahim', ln: 'Ouattara', g: 'MALE' as const, f: 0, l: 1, hasUser: false },
    { fn: 'Aminata', ln: 'Diallo', g: 'FEMALE' as const, f: 1, l: 0, hasUser: false },
    { fn: 'Ousmane', ln: 'Konan', g: 'MALE' as const, f: 1, l: 1, hasUser: false },
    { fn: 'Kadiatou', ln: 'Coulibaly', g: 'FEMALE' as const, f: 2, l: 0, hasUser: false },
    { fn: 'Mamadou', ln: 'Bamba', g: 'MALE' as const, f: 2, l: 1, hasUser: false },
    { fn: 'Mariam', ln: 'Konaté', g: 'FEMALE' as const, f: 0, l: 0, hasUser: false },
    { fn: 'Cheick', ln: 'Diabaté', g: 'MALE' as const, f: 0, l: 1, hasUser: false },
    { fn: 'Aïcha', ln: 'Sylla', g: 'FEMALE' as const, f: 1, l: 0, hasUser: false },
    { fn: 'Bakary', ln: 'Koné', g: 'MALE' as const, f: 1, l: 1, hasUser: false },
    { fn: 'Djénéba', ln: 'Traoré', g: 'FEMALE' as const, f: 2, l: 0, hasUser: false },
    { fn: 'Seydou', ln: 'Ouattara', g: 'MALE' as const, f: 2, l: 1, hasUser: false },
    { fn: 'Rokia', ln: 'Diallo', g: 'FEMALE' as const, f: 0, l: 0, hasUser: false },
    { fn: 'Amadou', ln: 'Konan', g: 'MALE' as const, f: 0, l: 1, hasUser: false },
    { fn: 'Bintou', ln: 'Coulibaly', g: 'FEMALE' as const, f: 1, l: 0, hasUser: false },
    { fn: 'Lassina', ln: 'Bamba', g: 'MALE' as const, f: 1, l: 1, hasUser: false },
    { fn: 'Fatou', ln: 'Konaté', g: 'FEMALE' as const, f: 2, l: 0, hasUser: false },
    { fn: 'Drissa', ln: 'Diabaté', g: 'MALE' as const, f: 2, l: 1, hasUser: false },
    { fn: 'Awa', ln: 'Sylla', g: 'FEMALE' as const, f: 0, l: 0, hasUser: false },
  ];

  const levelMap = [levels[0], levels[1], levels[2], levels[3], levels[4], levels[5]];

  const { students, classes } = await db.$transaction(async (tx) => {
    // Create 20 students (no classId yet)
    ...
    const c2 = await tx.class.upsert({
      where: { id: 'class-geco-l1' },
      update: {},
      create: {
        id: 'class-geco-l1',
        name: 'L1 Geco - Groupe A',
        levelId: levels[2].id,
        academicYearId: academicYear.id,
        capacity: 40,
        room: 'Salle B2',
    ...
    const c3 = await tx.class.upsert({
      where: { id: 'class-market-l1' },
      update: {},
      create: {
        id: 'class-market-l1',
        name: 'L1 Marketing - Groupe A',
        levelId: levels[4].id,
        academicYearId: academicYear.id,
        capacity: 35,
        room: 'Salle C3',
  ...
  console.log('✅ 20 étudiants créés');
  console.log('✅ 4 classes créées');

  // ========================
  // Batch 7: 6 PLANS DE PAIEMENT avec tranches
  // All plans are independent — nested create handles tranches within each plan
  // include: { tranches: true } so we can reference tranche IDs for payments
  // ========================
  const planConfigs = [
    ...
    { level: levels[2], name: 'L1 Geco - Standard', total: 360000, tranches: [
    ...
    { level: levels[3], name: 'L2 Geco - Standard', total: 360000, tranches: [
      { n: 1, name: 'Inscription', amount: 110000, due: '2024-10-15' },
      { n: 2, name: '1ère tranche', amount: 125000, due: '2025-01-15' },
      { n: 3, name: '2ème tranche', amount: 125000, due: '2025-04-15' },
    ]},
    { level: levels[4], name: 'L1 Marketing - Standard', total: 390000, tranches: [
    ...
    { level: levels[5], name: 'L2 Marketing - Standard', total: 390000, tranches: [
      { n: 1, name: 'Inscription', amount: 120000, due: '2024-10-15' },
      { n: 2, name: '1ère tranche', amount: 135000, due: '2025-01-15' },
      { n: 3, name: '2ème tranche', amount: 135000, due: '2025-04-15' },
    ]},
  ...
  console.log(`✅ ${paymentPlans.length} plans de paiement créés avec tranches`);

  // ========================
  // Batch 8: PAIEMENTS D'ÉCHANTILLON (~40 payments)
  // Clear existing payments first for idempotency
  // ========================
  await db.payment.deleteMany();

  const paymentMethods: Array<'CASH' | 'MTN_MOMO' | 'ORANGE_MONEY' | 'WAVE' | 'BANK_TRANSFER' | 'CHEQUE'> = [
    ...
    paymentOps.push(
      db.payment.create({
        data: {
          ...
          receivedBy: users[3].id, // caissier
          status: 'COMPLETED',
        },
      }),
    );
      ...
      paymentOps.push(
        db.payment.create({
          data: {
            ...
            receivedBy: users[2].id, // comptable
            status: 'COMPLETED',
          },
        }),
      );
  ...
  // ========================
  // Batch 9a: PLAN COMPTABLE OHADA (classes 1-8) — all independent upserts
  // ========================
  const ohadaAccounts = [
    ...
    ohadaAccounts.map(account =>
      db.chartOfAccount.upsert({
        ...
        create: {
          ...
          accountType: account.type as never,
          normalBalance: account.balance as never,
          isActive: true,
        },
      }),
    ),
  ...
  console.log(`✅ Plan comptable OHADA créé (${ohadaAccounts.length} comptes, classes 1-8)`);

  // ========================
  // Batch 9b: NOTIFICATIONS (clear and recreate for idempotency)
  // ========================
  ...
  console.log('📊 Résumé :');
  console.log(`   - 6 rôles (ADMIN, TEACHER, ACCOUNTANT, CASHIER, SECRETARY, STUDENT)`);
  console.log(`   - 6 utilisateurs de test`);
  console.log(`   - 164 permissions RBAC`);
  console.log(`   - 3 filières, 6 niveaux`);
  console.log(`   - 9 matières`);
  console.log(`   - 20 étudiants`);
  console.log(`   - 4 classes`);
  console.log(`   - ${paymentPlans.length} plans de paiement avec tranches`);
  console.log(`   - ${paymentCount} paiements enregistrés`);
  console.log(`   - ${ohadaAccounts.length} comptes OHADA (classes 1-8)`);
  console.log('\n🔑 Comptes de test :');
  console.log('   Admin:     admin@ipve.edu.ci / Admin@2025');
  console.log('Enseignant: m.konan@ipve.edu.ci / Teacher@2025');
  console.log('Comptable:  finance@ipve.edu.ci / Finance@2025');
  console.log('Caissier:   caisse@ipve.edu.ci / Caissier@2025');
  console.log('Secrétaire: secretariat@ipve.edu.ci / Secret@2025');
  console.log('Étudiant:   a.kone@etu.ipve.ci / Student@2025');
}
